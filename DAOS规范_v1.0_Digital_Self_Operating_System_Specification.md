# DAOS 规范 v1.0 — Digital Avatar Operating System Specification
...
- [第三章：架构规范](#第三章架构规范)
  ...
  - [3.9 第七层：分身进化层](#39-第七层分身进化层Avatar-evolution)
- [第四章：上下文管理规范](#第四章上下文管理规范)
- [第五章：组织层级规范](#第五章组织层级规范)
- [第六章：数字分身规范](#第六章数字分身规范)
- [第七章：本地优先与隐私规范](#第七章本地优先与隐私规范)
- [第八章：开发者生态规范](#第八章开发者生态规范)
- [第九章：合规性要求](#第九章合规性要求)
- [第十章：参考实现 — RClaw](#第十章参考实现--rclaw)
- [第十一章：术语表](#第十一章术语表)
- [附录 A：DAOS 与现有「智能体」产品的根本区别](#附录-aDAOS-与现有智能体产品的根本区别)
- [附录 B：DAOS 规范版本历史](#附录-bDAOS-规范版本历史)
- [附录 C：致谢](#附录-c致谢)
...
当前市场上被称为「智能体（Agent）」的产品，绝大多数本质上是带有工具调用能力的聊天窗口。它们的记忆受限于大语言模型（LLM）的上下文窗口大小，知识无法持久积累，行为无法分身进化，更无法真正成为用户的数字分身。

DAOS（Digital Avatar Operating System，数字分身操作系统）规范的提出，旨在重新定义什么是真正的智能体操作系统。DAOS 不是某一个产品的品牌，而是一套开放的技术规范——任何遵循此规范实现的系统，都可以称为 DAOS。市场上应该有医疗行业的 DAOS、教育行业的 DAOS、法律行业的 DAOS，它们遵循同一套核心规范，但各自适配不同领域。
...
- **DAOS** — Digital Avatar Operating System，数字分身操作系统
- **Digital Avatar** — 数字分身，虚拟了用户行为习惯的智能体实例
...
- **Avatar-Evolution** — 分身进化，审计回顾 → 技能沉淀的闭环机制
...
DAOS 的设计基于三层递进哲学，每一层建立在前一层之上：

**第一层：数字分身（Digital Avatar）**

Agent 不是工具，是虚拟了用户所有行为习惯的数字分身。它学习用户的说话方式、决策偏好、时间安排、协作风格，随着使用时间的增长越来越像用户。这不是预设的角色扮演，而是通过持续交互自然形成的个性化。
...
5. **分身进化原则** — Agent 应能通过审计回顾自动沉淀新技能
6. **开放规范原则** — DAOS 是开放技术规范，任何人均可实现
...
DAOS 采用七层架构 + 第零层安全基础的设计。每一层有明确的职责边界和接口定义。

```
┌─────────────────────────────────────────┐
│  第七层：分身进化层（Avatar-Evolution）      │
│  审计回顾 → 技能沉淀 → 能力增长           │
├─────────────────────────────────────────┤
...
- DAOS MUST 为每个设备生成唯一的设备身份密钥对
- DAOS MUST 实现用户强身份认证机制
- DAOS MUST 实现端到端加密通信
- 设备与服务端之间的握手加密（Handshake Encryption）
- 基于 WebSocket 的加密隧道
- 消息级别的加密传输
- DAOS MUST 实现完整的密钥生命周期管理
- DAOS SHOULD 支持多因素认证（MFA）
- DAOS MAY 支持生物特征认证
...
- DAOS MUST 为每个 Agent 实例提供独立的运行沙箱
- DAOS MUST 实现 Agent 间的 IPC 鉴权机制
- DAOS MUST 确保文件系统挂载的安全性
- 发送方身份验证（Sender Allowlist）
- 消息签名与完整性校验
- 通信通道的访问控制
- DAOS MUST 防止 Agent 间的非授权数据访问
- DAOS SHOULD 提供细粒度的资源访问控制
- DAOS MAY 支持跨容器资源调度
...
- DAOS MUST 实现技能管理系统
- DAOS MUST 支持工具的语义检索
- DAOS MUST 支持 MCP（Model Context Protocol）协议
- DAOS SHOULD 实现浏览器操控能力
- DAOS SHOULD 支持工具备注检索
- DAOS MAY 支持自定义工具运行时
...
- DAOS MUST 实现消息路由机制
- DAOS MUST 实现任务调度系统，至少支持：
  - 一次性定时任务（指定时间执行一次）
  - 周期性定时任务（Cron 表达式或固定间隔）
  - 任务的状态管理（创建、暂停、恢复、取消）
- DAOS MUST 实现队列管理
- DAOS SHOULD 支持多种上下文模式：
...
这是 DAOS 规范最核心的层，也是与传统「智能体」产品的根本区别所在。

规范要求：

- DAOS MUST 实现双记忆架构：工作上下文（固定大小）+ 长期记忆（向量数据库）
- DAOS MUST 为工作上下文设定固定上限
- DAOS MUST 在工作上下文中仅保留文件名引用，不保留文档全文
- DAOS MUST 使用向量数据库作为长期记忆存储
- DAOS MUST 实现自动入库机制，将新文档/图片向量化
- DAOS MUST 提供以下核心配置文件：
  - 角色描述文件（CLAUDE.md）
  - 记忆文件（MEMORY.md）
  - 规则文件（RULES.md）
  - 灵魂文件（SOUL.md）
- DAOS SHOULD 使用向量数据库语义分区，至少包含：
  - 对话记录表（conversations）
  - 文档表（documents）
  - 图片表（images）
  - 记忆表（memories）
...
- DAOS MUST 记录所有工具调用的审计日志
- DAOS MUST 使 Agent 行为对用户完全透明可见
- DAOS SHOULD 执行每日审计回顾
- DAOS SHOULD 提供行为质量评估指标
- DAOS MAY 支持自定义观测面板
...
- DAOS MUST 实现规则引擎，约束 Agent 行为
- DAOS MUST 在检测到约束违反时触发恢复机制
- DAOS SHOULD 在约束违反时执行以下恢复动作：
  - 自动回滚已执行的操作
  - 向用户报告约束违反详情
  - 提供替代方案建议
- DAOS SHOULD 支持规则动态更新
- DAOS MAY 支持约束优先级分级

### 3.9 第七层：分身进化层（Avatar-Evolution）

这是 DAOS 规范区别于所有现有 Harness 实现的核心创新层。

规范要求：

- DAOS MUST 实现每日审计回顾机制
- DAOS MUST 支持自动技能创建（识别高频操作模式）
- DAOS MUST 将创建的技能向量化存储到技能库
- DAOS SHOULD 支持技能版本管理
- DAOS SHOULD 在创建技能后通知用户
- DAOS MAY 支持技能共享与导入导出
...
- DAOS MUST 在工作上下文中仅保留文档的文件名引用
- DAOS MUST NOT 将文档全文塞入工作上下文
- 当 Agent 需要访问文档内容时，MUST 通过语义检索工具从向量数据库或文件系统获取
- 文件名引用 SHOULD 包含文档的基本元信息（文件名、类型、日期）

**原则三：外部记忆无限扩展**

- DAOS MUST 使用向量数据库作为长期记忆的存储后端
- DAOS MUST 实现文档和图片的自动向量化入库机制
- DAOS MUST 支持语义检索以从长期记忆中获取相关内容
...
### 4.5 为什么固定上下文是 DAOS 的根基

固定上下文不是妥协，而是 DAOS 实现「本地、透明、可控」的技术根基：
...
- DAOS MUST 确保 Agent 间的权限隔离
- DAOS SHOULD 支持 Agent 的组织分组管理
- DAOS SHOULD 支持 Agent 的层级深度至少为 3 层
...
- DAOS MUST 在用户离开组织时保留其 Agent 和所有数据
- DAOS MUST 支持将 Agent 的所有权转移给其他用户
- DAOS SHOULD 保留 Agent 的完整工作桌面和决策记录
- DAOS SHOULD 支持新成员继承前任 Agent 的知识
- DAOS MAY 支持 Agent 数据的归档与恢复
...
数字分身（Digital Avatar）是 DAOS 的核心概念。它不是预设的角色模板，而是通过持续交互自然形成的用户镜像。

数字分身与传统 AI 助手的区别：

| 维度 | 传统 AI 助手 | DAOS 数字分身 |
|------|-------------|--------------|
| 身份 | 通用工具 | 用户的数字镜像 |
| 个性化 | 预设角色模板 | 通过交互自然形成 |
| 学习方式 | 一次性配置 | 持续吸收用户风格 |
| 记忆 | 会话级，会话结束即遗忘 | 永久积累，越用越懂你 |
| 进化 | 静态，能力固定 | 分身进化，自动沉淀新技能 |
| 归属 | 平台所有 | 用户所有 |
...
- DAOS MUST 持续学习以下用户特征：
  - 用户的沟通风格和用词习惯
  - 用户的决策偏好和优先级
  - 用户的工作流程和操作模式
  - 用户的创作风格和审美倾向
- DAOS MUST 将学习到的特征持久化存储
- DAOS SHOULD 在交互中自适应调整行为风格
- DAOS SHOULD 支持用户手动校准数字分身
- DAOS MAY 支持多场景的数字分身切换

### 6.3 数字分身的可视化

规范要求：

- DAOS SHOULD 提供数字分身的可视化虚拟形象
- DAOS SHOULD 提供工作桌面的可视化界面
- DAOS MUST 在工作桌面上实时展示：
  - Agent 的思考过程
  - Agent 的工具调用及参数
  - 工具的返回结果
  - Agent 的决策推理链
...
- DAOS MUST 支持完全本地运行模式
- DAOS MUST 使用本地 AI 模型进行推理
- DAOS MUST 确保核心功能不依赖云端服务
- DAOS SHOULD 支持离线运行
- DAOS MAY 提供云端加速的可选方案

### 7.2 数据隐私规范

规范要求：

- DAOS MUST 默认不上传用户数据到云端
- DAOS MUST 在需要联网时明确告知用户
- DAOS MUST 提供数据导出功能
- DAOS SHOULD 提供数据本地备份方案
- DAOS MAY 提供端到端加密的数据同步
...
- DAOS MUST 提供开发者 SDK
- DAOS MUST 在 SDK 中封装以下核心流程：
  - Token 过期与自动刷新
  - 加密通信流程
  - 设备认证与握手
  - WebSocket 连接管理
- DAOS SHOULD 提供多语言 SDK（至少支持 TypeScript）
- DAOS MAY 提供 CLI 开发工具

### 8.2 主题/皮肤系统规范

规范要求：

- DAOS SHOULD 提供可扩展的主题系统
- DAOS SHOULD 提供主题开发模板
- DAOS MAY 支持主题市场/社区
...
**Level 1 — 基础合规（DAOS-Core）**

必须满足所有 MUST 级别的规范要求。实现 DAOS-Core 的系统可以使用「DAOS-Core Compliant」标识。

**Level 2 — 完整合规（DAOS-Full）**

必须满足所有 MUST 和 SHOULD 级别的规范要求。实现 DAOS-Full 的系统可以使用「DAOS-Full Compliant」标识。

**Level 3 — 卓越合规（DAOS-Excellence）**

必须满足所有 MUST、SHOULD 和 MAY 级别的规范要求。实现 DAOS-Excellence 的系统可以使用「DAOS-Excellence Compliant」标识。
...
RClaw 是 DAOS 规范 v1.0 的第一个完整参考实现，达到 DAOS-Full 合规等级。以下为 RClaw 对各规范条款的物理实现映射。

### 10.2 架构映射

| DAOS 规范层 | RClaw 实现 | 关键文件/模块 |
|------------|-----------|-------------|
...
| 第七层：分身进化 | 审计回顾 → 自动创建技能 → 向量化 | `VectorIPCService/agent/creator.py`, `skills.lance` |

### 10.3 技术栈映射

| DAOS 规范要求 | RClaw 技术选型 |
|--------------|---------------|
